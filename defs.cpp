...
// this function sets the seed for the random number generator to the current time
void srand48() {
    seed = (time(nullptr));
}
...
double drand48() {
    ...
    seed = (0x5DEECE66DLL * seed + 0xBLL) & ((1LL << 48) - 1);
    return static_cast<double>(seed) / (1LL << 48);

}


// this function converts a string to a card value
int card_value(int rank, int suit) {
    return primes[rank] | (rank << 8) | suit | (1 << (16 + rank));
}

// this function converts a card value to a string
std::string card_string(int card) {
    char suit;
    ...
    return std::string(1, rank[r]) + suit;
}
